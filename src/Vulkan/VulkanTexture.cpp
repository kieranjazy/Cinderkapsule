...
VkImage createTextureImage(const std::string texturePath, VkDevice& device, VkPhysicalDevice& physicalDevice, VkDeviceMemory& textureImageMemory, VkCommandPool& commandPool, VkQueue& graphicsQueue) {
	...
	stbi_uc* pixels = stbi_load(texturePath.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha); //make sur ethis works
...
VkImageView createImageView(VkImage& image, VkFormat format, VkImageAspectFlags aspectFlags, VkDevice& device) { //Change so that image view is contained in VkModel
...
VkImageView createTextureImageView(VkImage& image, VkDevice& device) {
	return createImageView(image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, device);
}
